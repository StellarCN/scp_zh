...
方便起见，我们用下标来区分属于特别的节点或消息的域。如果$v$是一个节点，那么用$b_v$，$p_v$，$p_v^{\prime}\ldots$表示\todo{图16}中描述的节点$v$的状态值$b,p,p^{\prime}\ldots$。我们还设置$z_v$为上文提到的$v$的下一个表决——即，如果$P\neq \mybm{0}$那么$z_v$为$P_v.x$否则是$v$的合成值。类似地，令$b_m,p_m,p^{\prime}_m,P_m,c_m$表示在\todo{图17}中所描述的由一个网络消息$m$包含的对应的域。
